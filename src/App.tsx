...
const App: React.FC = () => {
  ...
  // Not logged in? Show login / signup shell
  if (!user) {
    return <AuthShell />;
  }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 px-4 py-4 md:px-8 md:py-6">
      ...
      <div className="max-w-6xl mx-auto mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div className="flex items-center gap-3">
          {/* ... your existing Ecoschoolia logo block ... */}
        </div>

        <div className="flex items-center gap-3">
          {/* Only show teacher toggle if they are a teacher */}
          {user.role === "teacher" && (
          ...
          <button
            onClick={logout}
            className="text-[11px] rounded-full border border-slate-700 px-3 py-1 text-slate-400 hover:bg-slate-900"
          >
            {user.displayName || user.firebaseUser.email} · Log out
          </button>
        </div>
      </div>
  ...
  const recommended = [
    {
      ...
      description:
        "Practice adding and subtracting fractions using a mini-quest.",
      subject: "Math",
    ...
    },
    {
      ...
      description:
        "Watch and answer quick questions about evaporation & rain.",
      subject: "Science",
    ...
    >
      ...
      <Card className="relative overflow-hidden bg-gradient-to-r from-emerald-500 via-teal-500 to-sky-500 border-none text-white">
        ...
        <CardContent className="pt-2 relative">
          <p className="text-[11px] text-emerald-50/90 mb-1">
            Today&apos;s focus
          </p>
              ...
              {section.videos.map((video) => (
                ...
                >
                  ...
                  <div className="p-2.5 space-y-1">
                    ...
                    <p className="text-[10px] text-slate-400">{video.tag}</p>
...
const AssignmentsScreen: React.FC = () => {
  ...
  return (
    <motion.div
      ...
      className="space-y-3"
    >
      <h2 className="text-sm font-semibold text-slate-100 flex itemscenter gap-1.5">
